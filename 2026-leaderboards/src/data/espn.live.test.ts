...
live('live ESPN contract', () => {
  it('returns the full 2026 scoreboard and matches an independent group-stage aggregation', async () => {
    ...
    expect(payload.events?.length).toBeGreaterThanOrEqual(100)
    ...
    for (const event of payload.events ?? []) {
      if (!event.status?.type?.completed || event.season?.slug !== 'group-stage') continue
      ...
      for (const competitor of competitors) {
        ...
        else if (goalsFor === goalsAgainst) result.points += 1
      }
    }

    for (const assignment of teamAssignments) {
      const actual = standings.find((entry) => entry.espnName === assignment.espnName)
      const group = independent.get(assignment.espnName)
      expect(actual, assignment.espnName).toBeDefined()
      expect(actual!.goalsFor, assignment.espnName).toBeGreaterThanOrEqual(group!.goalsFor)
      expect(actual!.goalsAgainst, assignment.espnName).toBeGreaterThanOrEqual(group!.goalsAgainst)
      expect(actual!.points, assignment.espnName).toBeGreaterThanOrEqual(group!.points)
    }
  }, 30_000)
})
